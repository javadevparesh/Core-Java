...
public class D_TerminalOps {
    public static void main(String[] args) {
        ...
        // ## 3. reduce : Combines elements to produce a single result (combining by summing up) also called 'Accumulator'
        Optional<Integer> optionalInteger = list.stream().reduce(Integer::sum);
        System.out.println(optionalInteger.get());

        // ## 4. count (Self) ##
        long count = list.stream().filter(x -> x>2).collect(Collectors.toList()).size();
        System.err.println( "No of elements: " + count);
        ...
        System.out.println("Squaring & Sorting: " + nums.stream().map(n -> n*n).sorted().toList());
        ...
        // ## 8. min / max ##
        System.out.println("max: " + Stream.of(2, 44, 69).max((o1, o2) -> o2 - o1).get()); // Reverse order
